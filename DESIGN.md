...
**The Map Rides With Content Rule.** The map (see Environmental Layer below) is deliberately normal-flow, not viewport-pinned: earlier revisions tried `position: fixed` and `position: absolute` variants to keep it visually anchored while `main` scrolled, and on real iOS/WebKit devices both still visibly dragged during the touch-scroll gesture - a long-documented class of WebKit bug where an active touch-scroll elsewhere on the page can drag fixed/absolute layers with it, no GPU-layer hinting reliably prevents it. The fix was architectural, not another CSS property: the map is `position: static`, the first child of `main`, so it scrolls exactly like any other content and behaves identically on every engine because it never asks the browser to do special work to keep it pinned. Do not reintroduce `position: fixed`/`absolute` (or transform/will-change hints in service of one) for this element - if a pinned-background effect is ever wanted again, it needs a real design discussion about the tradeoff, not a CSS patch.

## Environmental Layer

A single SVG (`.map-bg`) sits as the first child of `main`, above the itinerary/transport/checklist content, on every tab - a `340px`-tall banner a reader scrolls past once and then leaves behind, not a persistent backdrop. (An earlier revision made it a persistent fixed/absolute full-viewport layer; that was replaced by the Map Rides With Content Rule above.) It reads as a real railway map at a glance, not an abstract texture: a radial navy vignette and `feTurbulence` paper-grain (aged/printed, not a clean digital gradient) sit behind recognizable shapes - a faint dashed lat/long graticule; gestural Atlantic/North Sea coastline silhouettes with a very low-opacity landmass fill (not bare outlines); a schematic Italian peninsula (boot silhouette) anchoring the Rome end of the route; a row of small triangular hachure marks standing in for the Alps between Interlaken and Vienna, a genuine vintage-map mountain convention; and a denser mesh of thin secondary rail lines radiating from the main cities, so the highlighted route reads as one line picked out of a real network rather than the only line that exists. The primary route (Amsterdam → Paris → Interlaken → Vienna → Budapest → Rome) is a bolder dashed line with small amber waypoint ticks along it, terminating at open-square "station" markers - each paired with that city's own hue as a small filled dot, a short station tick, and its name in spaced small-caps mono. The full SVG (`viewBox="0 0 460 900"`) is anchored to its top edge (`preserveAspectRatio="xMidYMin slice"`) so the banner shows Amsterdam/Paris and the start of the route rather than an arbitrary crop. A small compass rose sits in one corner. Every value still stays low-contrast enough that shape and line work carry the "this is a map" read, while text and fill opacities stay conservative - texture and journey context, never a legible or functional map, and never strong enough to compete with the checklist/itinerary/transport content sharing the same scroll.
