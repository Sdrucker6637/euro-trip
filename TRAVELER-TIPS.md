...
scripts/traveler-tips/
  ...
  lib/reddit.mjs             <- Reddit evidence via Arctic Shift (free, keyless)
...
never blocks the others:
   Reddit search via Arctic Shift (7 query variants x 3 curated
   subreddits, since Arctic Shift has no sitewide search) + top
...
- Reddit (Arctic Shift) is unreachable, rate-limited, or a query fails
  → warned, skipped, other sources continue.
...
Reddit needs no key or registration at all - see the "Reddit" row below.

## APIs, keys, and cost

| Source | Key needed | Billing required | Notes |
|---|---|---|---|
| Reddit (via Arctic Shift) | None | No | `www.reddit.com/search.json` and `old.reddit.com/search.json` were tried first since they need no key - both reliably 403/404 plain HTTP-client requests (Reddit's anti-scraping bot detection fingerprinting the client, not a headers/rate-limit issue - confirmed by the exact same URL returning real results in a browser but failing from Node's fetch every time). Reddit's own OAuth API is the sanctioned fix for that but requires registering a developer app and accepting Reddit's Developer/Data API Terms, which was real friction for a personal project. Settled on [Arctic Shift](https://github.com/ArthurHeitmann/arctic_shift) instead - a free, keyless, community-run mirror of Reddit's public data (the de facto successor to Pushshift, which Reddit cut off in 2023). It's not an officially Reddit-sanctioned or guaranteed-uptime service, so treat it as best-effort, same as every other source here - if it's ever down or rate-limits, YouTube + official evidence still flows and nothing gets overwritten with emptiness. Arctic Shift also has no sitewide keyword search (its `query` param requires a `subreddit` or `author` filter, and `subreddit` only takes one value per request) - `lib/reddit.mjs` scopes each query to a small, hand-picked list of general travel subreddits (`SUBREDDITS` - same curated-not-guessed approach as `OFFICIAL_SITES`), which will miss a hit that only exists in a landmark-specific subreddit not on that list. It also soft-rate-limits aggressively: the first request in a run succeeds, then every request after it 422s ("Timeout. Maybe slow down a bit") at a 600ms pace - `REQUEST_DELAY_MS` in `lib/reddit.mjs` paces requests 3 seconds apart with one retry on a 422, which makes a full-itinerary run noticeably slower (~a few minutes per activity between Reddit's 21 search queries + comment fetches) but fine for a once-a-month job. |
...
Actions**: `GEMINI_API_KEY`, `YOUTUBE_API_KEY` (Reddit needs none). The
job commits `data/traveler-tips.json` back to the repo only if it
...
- **Run for real** against `eiffel-tower`, `palace-of-versailles`,
  `trevi-fountain` (YouTube + official; Reddit was mid-migration
  between three different approaches at the time - see the Reddit row
  above) - real evidence in, a real Gemini call, correctly-computed
  confidence, correctly-cited sources, all the way through. That output
  (`data/traveler-tips.json`) is committed and was verified rendering
  correctly in the actual app UI. Caught and fixed one real bug this
  way: the YouTube API returns titles/descriptions HTML-entity-encoded,
  which `index.html`'s `escapeHtml()` was double-encoding into visible
  `&amp;amp;` text - fixed in `lib/decode-html-entities.mjs` (also
  applied to Reddit's API, which has the same behavior).
- **Not yet run end-to-end against Arctic Shift** - it's a third-party
  service this pipeline doesn't control, so the first real run against
  it (from a normal machine or CI, not a sandboxed environment that may
  itself have restricted egress) is the actual test of whether it
  returns useful results at the volume/query pattern this pipeline uses.
