...
### Named Rules
**The Single-Scroll Rule.** `.app` never scrolls as a document; `main` is the one internal scroll container (`flex: 1; min-height: 0; overflow-y: auto`). Header, nav, and the background map layer all stay visually anchored to the viewport while only `main`'s content moves - the fixed bottom nav and fixed map layer read as an intentional app shell, not detached elements that happen to float over independently-scrolling content. A regression here (e.g. giving `.app` `min-height` instead of a fixed `height`, letting it grow past the viewport) breaks this silently: the whole document starts scrolling instead of `main`, and the header drifts out of sync with the nav/map that stay viewport-pinned.

## Environmental Layer

A single fixed, full-viewport SVG (`.map-bg`, `position: fixed; inset: 0; z-index: 0; pointer-events: none`) sits behind the entire app shell (`z-index: 1`, transparent background) and never scrolls - it is the view "outside the train window," constant while `main` scrolls the itinerary over it. It is deliberately built to read as a real railway map at a glance, not an abstract texture: a radial navy vignette and `feTurbulence` paper-grain (aged/printed, not a clean digital gradient) sit behind recognizable shapes - a faint dashed lat/long graticule; gestural Atlantic/North Sea coastline silhouettes with a very low-opacity landmass fill (not bare outlines); a schematic Italian peninsula (boot silhouette) anchoring the Rome end of the route; a row of small triangular hachure marks standing in for the Alps between Interlaken and Vienna, a genuine vintage-map mountain convention; and a denser mesh of thin secondary rail lines radiating from the main cities, so the highlighted route reads as one line picked out of a real network rather than the only line that exists. The primary route (Amsterdam → Paris → Interlaken → Vienna → Budapest → Rome) is a bolder dashed line with small amber waypoint ticks along it, terminating at open-square "station" markers - each paired with that city's own hue as a small filled dot, a short station tick, and its name in spaced small-caps mono. Labels are positioned (with a short leader line where needed, e.g. Paris) to avoid the band directly under the header, where every panel's first heading sits - the one place a label would read as a mistake instead of atmosphere. A small compass rose sits in one corner. Every value still stays low-contrast enough that shape and line work carry the "this is a map" read, while text and fill opacities stay conservative - texture and journey context, never a legible or functional map, and never strong enough to compete with foreground content for a reader's attention at any scroll position.
