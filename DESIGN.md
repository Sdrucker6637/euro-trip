...
A fixed-width mobile shell (`max-width: 460px`) centered on the page even on a wide desktop viewport - the design deliberately never adapts to a wider column; it always reads as a phone screen. `.app` is a true fixed-height shell (`height: 100dvh`, `overflow: hidden`) so it never grows past the viewport - a structural requirement, not a stylistic preference (see the Named Rule below). Structure is a vertical flex column: a non-scrolling header (route title + timeline), a scrolling main content area (the sole internal scroll container - `16px` side padding, `90px` bottom padding to clear the nav), and a fixed bottom tab bar (3 tabs) with safe-area-aware bottom padding for notched devices.
...
### Named Rules
**The Single-Scroll Rule.** `.app` never scrolls as a document; `main` is the one internal scroll container (`flex: 1; min-height: 0; overflow-y: auto`). Header and nav stay visually anchored to the viewport while only `main`'s content moves - the fixed bottom nav reads as an intentional app shell, not a detached element floating over independently-scrolling content. A regression here (e.g. giving `.app` `min-height` instead of a fixed `height`, letting it grow past the viewport) breaks this silently: the whole document starts scrolling instead of `main`, and the header drifts out of sync with the nav.

**The Map Rides With Content Rule.** The map (see Environmental Layer below) is deliberately positioned relative to `main`'s own scrolling content, not the viewport: earlier revisions tried `position: fixed` and `position: absolute`-relative-to-`body` to keep it visually anchored while `main` scrolled, and on real iOS/WebKit devices both still visibly dragged during the touch-scroll gesture - a long-documented class of WebKit bug where an active touch-scroll elsewhere on the page can drag fixed/absolute-to-a-non-scrolling-ancestor layers with it, no GPU-layer hinting reliably prevents it. The fix that actually holds: the map is `position: absolute` with `main` itself (not `body`) as its containing block, so its containing block *is* the thing that scrolls - the map moves through the exact same coordinate system as every checklist/itinerary/transport row, by construction, on every engine, because there is nothing browser-specific being asked of it. `main` and `.panel` both carry explicit `z-index` values so the map's stacking stays contained inside `main` and never escapes to compete with `header`/`nav` (a real regression hit once during development: an unscoped `z-index` on `.panel` briefly let scrolled content paint over the fixed nav and block its clicks). Do not reintroduce `position: fixed`, or `position: absolute` anchored to `body`/the viewport, for this element - if a pinned-background effect is ever wanted again, it needs a real design discussion about the tradeoff, not a CSS patch. Size it with a plain explicit `height` too, not `aspect-ratio` - an `aspect-ratio`-computed height on this element rendered correctly in Chromium but not on real Safari/WebKit; explicit pixel dimensions are the more universally reliable choice here even though they're less elegant.

## Environmental Layer

A single SVG (`.map-bg`) is absolutely positioned over the top of `main`'s content (an explicit `850px` height rather than a computed `aspect-ratio` - deliberately the plainer, more universally-supported property, after an `aspect-ratio`-sized version proved unreliable on real Safari/WebKit even though it computed correctly in Chromium), behind the itinerary/transport/checklist rows on every tab, with `pointer-events: none` so it never intercepts a tap. Because its containing block is `main` itself, it scrolls in lockstep with the content stacked on top of it - drag the checklist down and the map moves up by the same pixel amount, the same way a photo behind glass would move if you slid the glass. Every app-chrome row type that can sit directly over it - `day-card`, `nearby-item`, and `check-item` alike - stays on translucent glass (see Elevation & Depth), so the map is always seen through a frosted layer, never as bare, crisp linework sitting under plain text; a `check-item` without that glass backing (an earlier state of the checklist) let the route line, city labels, and Alps hachures render at full clarity directly across list rows, which read as stray clutter rather than atmosphere and is exactly what the glass rule above exists to prevent. It reads as a real railway map at a glance, not an abstract texture: a radial navy vignette sits behind recognizable shapes - a faint dashed lat/long graticule; gestural Atlantic/North Sea coastline silhouettes with a very low-opacity landmass fill (not bare outlines); a schematic Italian peninsula (boot silhouette) anchoring the Rome end of the route; a row of small triangular hachure marks standing in for the Alps between Interlaken and Vienna, a genuine vintage-map mountain convention; and a denser mesh of thin secondary rail lines radiating from the main cities, so the highlighted route reads as one line picked out of a real network rather than the only line that exists. The primary route (Amsterdam → Paris → Interlaken → Vienna → Budapest → Rome) is a bolder dashed line with small amber waypoint ticks along it, terminating at open-square "station" markers - each paired with that city's own hue as a small filled dot, a short station tick, and its name in spaced small-caps mono. A small compass rose sits in one corner. Every value still stays low-contrast enough that shape and line work carry the "this is a map" read, while text and fill opacities stay conservative - texture and journey context, never a legible or functional map, and never strong enough to compete with whatever content is currently scrolled over it. An earlier revision also layered a `feTurbulence` fractal-noise filter over the whole map for paper grain; it was removed - at `0.05` opacity its visual contribution was negligible, while SVG turbulence filters are a known WebKit scroll-performance hazard (they commonly force software repaint of their containing layer on every scroll frame), a much likelier explanation for reported scroll stutter than any single cost/benefit case justified keeping.
...
### Day Timeline (activity list)
A day's stops are not stacked cards - they're rows on a small vertical rail, a per-day miniature of the header's own route-line motif. Each `stop-row` is indented `26px` from a continuous `1px` line at the left edge; a small `9px` hollow circle with a `2px` amber ring marks each stop against that line, and the line runs seamlessly from the first stop's node to the last (it starts and ends at the node, never floating above the first or trailing past the last). The stop name (`14px`/600) and its description (`12px`, muted) keep a clear one-two weight step, so the row is scannable as "what" then "why it's here" without needing a numbered label - the node on the rail already carries the sequence. This replaces a flat dotted divider between rows; it is deliberately *not* another rounded card per activity, which would just nest cards inside a card.
